...
class HumanPlayer(Player):
    ...
    def get_move(self,game):
        ...
        while not valid_square:
            square = input(self.letter + '\'s turn. Inpute move (0-8): ')
            try:
                val = int(square)
...
class GeniousComputerPlayer(Player):
    ...
    def minimax(self,state,player):
        ...
        for possible_move in state.available_moves():
            ...
            #step 2 - recurse using minmay to simulate a game after making that move
            sim_score = self.minimax(state,other_player)
            #step 3 - undo the move
            state.board[possible_move] = ' '
            state.current_winner = None
            sim_score['position']=possible_move
            #step 4 - uptade the dictionaries if it is neccesary
            if player == max_player: #maximize yours
